...
- the **probability measure** $P : \mathcal{F} \to [0,1]$: a measure with the property that $P(\Omega) = 1$.
...
- The **cumulative distribution function (CDF)** $F_X : \mathbb{R} \to [0,1]$,
   $$F_X(x) := P(X \le x) = \int_{-\infty}^x f_X(w)\,dw.$$
  The probability that…
    - ...$X$ is at most $x$.
    - ...event happens on or before $x$.

- The **probability density function (PDF)** $f_X : \mathbb{R} \to [0,\infty)$
   $$f_X = F_X', \qquad F_X(x) = \int_{-\infty}^x f_X(w)\,dw, \qquad f_X(x) = \lim_{h\to 0^+} \frac{P_X([x,x+h])}{h}.$$
